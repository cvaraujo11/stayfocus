...
export default function ConcursosPage() {
  ...
  const { concursos, loading, error, carregarConcursos, adicionarConcurso, setupRealtimeSync } = useConcursosStore();
  ...
  const handleImportConcurso = async (concurso: any) => {
    try {
      await adicionarConcurso(concurso);
      // The new concurso will be added to the store, get the first one
      // Note: In a real scenario, we'd want to return the ID from adicionarConcurso
      setShowImportModal(false);
    } catch (error) {
      console.error('Error importing concurso:', error);
    }
  };

  // Show loading state
  if (authLoading || (loading && concursos.length === 0)) {
    return (
