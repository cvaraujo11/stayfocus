...
export default function DetalhesConcursoPage({ params }: DetalhesConcursoPageProps) {
  ...
  const { concursos, loading, error, carregarConcursos, removerConcurso, atualizarProgresso, setupRealtimeSync } = useConcursosStore();
  const { carregarQuestoes, setupRealtimeSync: setupQuestoesSync } = useQuestoesStore();
  ...
  // Load concursos on mount
  useEffect(() => {
    if (user && concursos.length === 0) {
      carregarConcursos(user.id);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, concursos.length]);

  // Load questoes for this concurso
  useEffect(() => {
    if (user && params.id) {
      carregarQuestoes(user.id, params.id);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, params.id]);

  // Setup real-time sync for concursos
  useEffect(() => {
    if (user) {
  ...
  // Setup real-time sync for questoes
  useEffect(() => {
    if (user) {
      const cleanup = setupQuestoesSync(user.id);
      return cleanup;
    }
  }, [user, setupQuestoesSync]);
  ...
  const handleRemover = async () => {
    if (confirm('Tem certeza que deseja remover este concurso?')) {
      try {
        await removerConcurso(concurso.id);
