...
export function RegistroSono() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modoEdicao && idEdicao) {
        await atualizarRegistroSono(idEdicao, {
          inicio: inicioISO,
          fim: fimISO,
          qualidade,
          notas
        ...
        await adicionarRegistroSono(inicioISO, fimISO, qualidade, notas)
      }
  ...
  const registrarAcordarAgora = async (id: string) => {
    try {
      ...
      await atualizarRegistroSono(id, { fim: agora })
    } catch (error) {
      console.error('Erro ao registrar acordar:', error)
    }
  }
