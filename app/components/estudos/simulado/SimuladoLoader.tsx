...
import { Upload, ClipboardPaste, PlayCircle } from 'lucide-react';

const SimuladoLoader: React.FC = () => {
  const { loadSimulado, setStatus, loading: storeLoading, error: storeError } = useSimuladoStore();
  const { concursos } = useConcursosStore();
  const { buscarQuestoesPorConcurso } = useQuestoesStore();
    ...
    try {
      ...
      const simuladoData: SimuladoData = {
        ...
        questoes: questoesSelecionadas.map((q, index) => {
          ...
          return {
            id: (index + 1).toString(), // Converte para string
  ...
  if (storeLoading || isLoading) {
    return (
      <div className="p-6 border rounded-lg shadow-sm bg-card text-card-foreground">
        ...
        <p className="text-center mt-4">Carregando simulado...</p>
      </div>
    );
  }
