...
import { BookOpen, Plus, X, Edit, Trash, Check, Clock, AlertCircle } from 'lucide-react'
...
export function RegistroEstudos() {
  ...
  const handleAdicionarSessao = async () => {
    if (!novaSessao.titulo || submitting) return

    setSubmitting(true)
    try {
      await adicionarSessao(novaSessao)
  ...
  const salvarEdicao = async () => {
    if (!editandoId || !novaSessao.titulo || submitting) return

    setSubmitting(true)
    try {
      await editarSessao(editandoId, novaSessao)
  ...
  const handleRemoverSessao = async (id: string) => {
  ...
  if (loading && sessoes.length === 0) {
    return (
      <div className="flex items-center justify-center py-12">
        <LoadingSpinner size="lg" label="Carregando registros de estudo..." />
      </div>
    )
  }
  ...
  if (error) {
    return (
      <ErrorMessage
        message={error}
        onRetry={() => user && carregarRegistros(user.id)}
      />
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {sessoes.map((sessao) => (
          <div
            key={sessao.id}
            className={`p-3 bg-white dark:bg-gray-800 rounded-lg border ${sessao.completo
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start">
                <button
                  ...
                  className={`mt-1 mr-3 flex-shrink-0 w-5 h-5 rounded-full border ${sessao.completo
                ...
                <div>
                  <h3
                    className={`font-medium ${sessao.completo
              ...
              <div className="flex space-x-1">
                ...
                <button
                  onClick={() => handleRemoverSessao(sessao.id)}
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                disabled={!novaSessao.titulo || submitting}
                className="px-4 py-2 bg-estudos-primary text-white rounded-lg hover:bg-estudos-primary/90 focus:outline-none focus:ring-2 focus:ring-estudos-primary focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
