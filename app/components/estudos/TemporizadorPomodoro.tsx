...
import { Play, Pause, RefreshCw, Settings, X, Award } from 'lucide-react'
...
interface TemporizadorPomodoroProps {
  ...
  tarefa?: string // Tarefa sendo trabalhada
}

export function TemporizadorPomodoro({ tipo = 'geral', titulo, tarefa }: TemporizadorPomodoroProps) {
  const { user } = useAuth()
  const {
    configuracao,
  ...
  const cicloRef = useRef(ciclo)
  const ciclosCompletosRef = useRef(ciclosCompletos)
  const configuracaoRef = useRef(configuracao)
  ...
  useEffect(() => {
    ...
    ciclosCompletosRef.current = ciclosCompletos
    configuracaoRef.current = configuracao
    userRef.current = user
    tarefaRef.current = tarefa
  }, [ciclo, ciclosCompletos, configuracao, user, tarefa])

  // Carregar sessões do dia atual
  useEffect(() => {
    if (user) {
      const hoje = new Date().toISOString().split('T')[0]
      carregarSessoes(user.id, hoje)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user])

  // Configurar sincronização em tempo real (separado para evitar loops)
  useEffect(() => {
    if (user) {
      ...
      interval = setInterval(() => {
        setTime((time) => {
          if (time <= 1) {
            ...
            // Registrar sessão concluída no banco de dados usando refs
            const currentCiclo = cicloRef.current
            const currentConfig = configuracaoRef.current
            const currentUser = userRef.current
            const currentTarefa = tarefaRef.current

            const duracaoMinutos = currentCiclo === 'foco'
              ? currentConfig.tempoFoco
              : currentCiclo === 'pausa'
                ? currentConfig.tempoPausa
                : currentConfig.tempoLongapausa

            const tipoSessao = currentCiclo === 'foco' ? 'foco' : 'pausa'
            ...
            // Alternar entre ciclos usando refs
            const currentCiclosCompletos = ciclosCompletosRef.current

            if (currentCiclo === 'foco') {
              incrementarCiclosCompletos()

              // Verificar se deve ser uma pausa longa
              if ((currentCiclosCompletos + 1) % currentConfig.ciclosAntesLongapausa === 0) {
                setCiclo('longapausa')
                return currentConfig.tempoLongapausa * 60
              } else {
                setCiclo('pausa')
                return currentConfig.tempoPausa * 60
              }
            } else {
              setCiclo('foco')
              return currentConfig.tempoFoco * 60
            }
          }
          return time - 1
        })
  ...
  }, [isActive, isPaused, incrementarCiclosCompletos, registrarSessao])
