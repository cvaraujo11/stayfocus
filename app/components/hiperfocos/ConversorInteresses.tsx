...
export function ConversorInteresses() {
  const { user } = useAuth()
  const { adicionarHiperfoco, adicionarTarefa, carregarHiperfocos, loading } = useHiperfocosStore()
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const hiperfocoId = await adicionarHiperfoco(
        formData.titulo,
        formData.descricao,
  ...
  return (
    <div>
      ...
      {feedback && (
        <div
          className={`mb-4 p-3 rounded-md ${feedback.tipo === 'sucesso'
          ...
          <div className="flex gap-2 flex-wrap">
            {CORES_HIPERFOCOS.map((cor) => (
              <button
                ...
                className={`w-8 h-8 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-hiperfocos-primary ${formData.corSelecionada === cor ? 'ring-2 ring-offset-2 ring-hiperfocos-primary' : ''
                  }`}
