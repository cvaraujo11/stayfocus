...
export function AdicionarReceitaForm({ receitaParaEditar, aoFinalizar }: AdicionarReceitaFormProps) {
  ...
  const { adicionarReceita, atualizarReceita, loading, error } = useReceitasStore();
  ...
  const salvarReceita = async (e: FormEvent) => {
    ...
    try {
      ...
      const receitaData: Omit<Receita, 'id' | 'favorita'> = {
        nome: receita.nome,
        descricao: receita.passos.filter(p => p.trim() !== '').join('\n'),
        ...
        ingredientes: receita.ingredientes.map(ing => ({
          ...ing,
          quantidade: parseFloat(ing.quantidade.replace(',', '.')) || 0
        })),
      ...
      if (editando && receitaParaEditar) {
        ...
        if (aoFinalizar) {
          aoFinalizar({ ...receitaData, id: receitaParaEditar.id, favorita: receitaParaEditar.favorita });
        } else {
        ...
        if (aoFinalizar) {
          aoFinalizar({ ...receitaData, id: 'temp', favorita: false });
        } else {
  ...
  if (!user) {
    return (
      <div className="p-4 text-center">
        <ErrorMessage message="Você precisa estar autenticado para acessar esta página." />
      </div>
    );
  }
  ...
  return (
    <form onSubmit={salvarReceita} className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex justify-end gap-4 pt-6 border-t">
        <Button type="button" onClick={handleCancel} variant="outline" disabled={submitting || loading}>
          Cancelar
        </Button>
        <Button type="submit" color="primary" disabled={submitting || loading}>
          {submitting || loading ? 'Salvando...' : editando ? 'Atualizar Receita' : 'Salvar Nova Receita'}
        </Button>
      </div>
    </form>
  );
