...
export function QuestaoList({ concursoId, onAddQuestao, onEditQuestao }: QuestaoListProps) {
  const { questoes, removerQuestao, loading, error } = useQuestoesStore((state) => ({
              ...
              <Card key={questao.id} className="p-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2 flex-shrink-0 ml-4">
                    <Button variant="outline" size="icon" onClick={() => onEditQuestao(questao)} disabled={removendo === questao.id}>
                      <Edit size={16} />
                    </Button>
                    <Button variant="destructive" size="icon" onClick={() => handleRemover(questao.id)} disabled={removendo === questao.id}>
                      {removendo === questao.id ? '...' : <Trash size={16} />}
                    </Button>
                  </div>
                </div>
              </Card>
