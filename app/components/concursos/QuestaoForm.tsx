...
export function QuestaoForm({ isOpen, onClose, concursoId, questaoParaEditar }: QuestaoFormProps) {
  const { adicionarQuestao, atualizarQuestao, loading, error } = useQuestoesStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (questaoParaEditar) {
        ...
        await atualizarQuestao(questaoParaEditar.id, questaoData);
      } else {
        // Passa os dados sem o ID para adicionar
        // questaoParaAdicionar já está formatado corretamente
        await adicionarQuestao(questaoParaAdicionar);
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-4">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? 'Salvando...' : questaoParaEditar ? 'Salvar Alterações' : 'Adicionar Questão'}
          </Button>
        </div>
      </form>
    </Modal>
