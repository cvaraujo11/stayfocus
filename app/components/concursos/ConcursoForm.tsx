...
export function ConcursoForm({ isOpen, onClose, concursoParaEditar }: ConcursoFormProps) {
  const { adicionarConcurso, atualizarConcurso, loading, error } = useConcursosStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (concursoParaEditar) {
        await atualizarConcurso(concursoParaEditar.id, formData);
      } else {
        await adicionarConcurso(formData);
      }
      onClose();
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-4">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? 'Salvando...' : concursoParaEditar ? 'Salvar Alterações' : 'Adicionar Concurso'}
          </Button>
        </div>
      </form>
    </Modal>
