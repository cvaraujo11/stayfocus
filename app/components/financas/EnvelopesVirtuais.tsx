...
export function EnvelopesVirtuais() {
  const { user } = useAuth()
  const { envelopes, adicionarEnvelope, atualizarEnvelope, removerEnvelope, registrarGastoEnvelope, carregarDados, loading, error } = useFinancasStore()
  ...
  const handleRegistrarGasto = async (id: string) => {
    if (!valorGasto || !valorGasto.valor || valorGasto.valor <= 0) return

    await registrarGastoEnvelope(id, valorGasto.valor)
    if (!error) {
      setValorGasto(null)
    }
  }
              ...
              <div className="flex flex-wrap gap-2">
                {coresPredefinidas.map(cor => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full ${novoEnvelope.cor === cor ? 'ring-2 ring-offset-2 ring-gray-500' : ''
                      }`}
