...
import { Check, CreditCard, PlusCircle, AlertCircle } from 'lucide-react'
import { LoadingSpinner } from '@/app/components/common/LoadingSpinner'
import { ErrorMessage } from '@/app/components/common/ErrorMessage'

export function AdicionarDespesa() {
  const { user } = useAuth()
  const { categorias, adicionarTransacao, carregarDados, loading, error } = useFinancasStore()
  ...
  const handleAdicionarDespesa = async (e: React.FormEvent) => {
    ...
    // Only show feedback if no error
    if (!error) {
  ...
  return (
    <div className="space-y-4">
      {error && (
        <ErrorMessage
          message={error}
          onRetry={() => user && carregarDados(user.id)}
        />
      )}
      ...
      {mostrarFeedback ? (
        ...
        <form onSubmit={handleAdicionarDespesa} className="space-y-4">
            ...
            <div className="grid grid-cols-5 gap-2 mt-2">
              {categorias.map(categoria => (
                <button
                  ...
                  className={`h-14 p-2 rounded-md flex flex-col items-center justify-center text-xs ${categoriaId === categoria.id
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full px-4 py-3 mt-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {submitting ? (
              <>
