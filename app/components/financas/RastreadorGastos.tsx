...
export default function RastreadorGastos() {
  const { user } = useAuth()
  const { categorias, transacoes, carregarDados, loading, error } = useFinancasStore()
  ...
  useEffect(() => {
    ...
    despesas.forEach(transacao => {
      const categoriaId = transacao.categoriaId || 'sem-categoria'
      if (!gastosPorCategoria[categoriaId]) {
        gastosPorCategoria[categoriaId] = 0
      }
      gastosPorCategoria[categoriaId] += transacao.valor
    })
  ...
  if (error) {
    return <ErrorMessage message={error} onRetry={() => user && carregarDados(user.id)} />
  }
